...
글 옆에 두고 상대 경로로 부른다. Astro가 크기·형식을 바꿔 내보낸다.

![WCAG 대비 측정 결과](./_media/contrast.png)

## 2. 애니메이션 GIF — 최적화 경로

같은 방식으로 GIF를 넣으면 어떻게 되는가.

![애니메이션 GIF, 최적화 경로](./_media/frames.gif)

## 3. 애니메이션 GIF — `public/` 경로

같은 파일을 `public/`에 두고 절대 경로로 부른다. 이쪽은 손대지 않고 그대로 나간다.
...
- **`public/`에 두면 손대지 않고 그대로 나간다.** 원본을 그 바이트 그대로
  내보내야 할 때만 쓴다.
